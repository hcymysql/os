...
BEGIN { 
system("rpm -Uvh https://mirrors.tuna.tsinghua.edu.cn/epel/6/x86_64/epel-release-6-8.noarch.rpm");
system("yum install -y vim ntpdate lrzsz chkconfig libaio* numa* jemalloc* perl-DBD-MySQL perl-Config-Tiny perl-Log-Dispatch perl-Parallel-ForkManager perl-ExtUtils* perl-CPAN*");
}
...
@server_name=split('\n',`chkconfig --level 345 --list|awk '{print \$1}'`);

print "默认打开crond,network,rsyslog,sshd服务,其他的服务初始化会关闭.\n\n";
...
foreach $sn (@server_name){
    switch($sn){
        case "crond"     { `chkconfig $sn on`; }
        case "rsyslog"   { `chkconfig $sn on`; }
        case "sshd"      { `chkconfig $sn on`; }
		case "network"   { `chkconfig $sn on`; }
        else  { print "关闭服务: $sn\n";
                system("chkconfig $sn off"); 
               }
    }}
print "-----------------\n";
system("chkconfig --level 345 --list | awk '\$4~/启用/'");
last;
}
